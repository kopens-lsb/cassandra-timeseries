...
**Upstream tracking rule:** `master` must be kept merged with the latest `cassandra-6.0` branch of GitHub `apache/cassandra` (remote `upstream`). Recurring conflict spots: `CHANGES.txt`, `debian/changelog`, `README.asc` (deleted in this fork so
