...
```sql
SELECT count(*) FROM pp.tm_tag_point;        -- → 0 (경고는 뜨지만 값은 0)
```

**부분 결과가 아니라 아무것도 안 보입니다.** 클라이언트 경고가 나가지만 대부분의 드라이버는 이를
...
- **질의 성능은 동률~30% 손해**입니다 ([벤치마크](tiering-benchmark.md)). 단, `hot_window` 안쪽만
  보는 질의는 병합을 건너뛰므로 **비용이 없습니다** — 손해는 콜드를 건드리는 질의만 냅니다.
