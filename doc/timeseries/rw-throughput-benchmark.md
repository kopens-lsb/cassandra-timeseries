...
## 구성 비교 — 시계열 스택(memtable+TSCS)과 계층화까지 켜면

같은 하네스로 **기본 구성 vs 시계열 스택 vs 계층화 후**를 비교했다 (2026-08-03 추가 측정,
10M 행 기준. 쓰기 최적화의 전말은 [memtable-write-tuning.md](memtable-write-tuning.md)):

| 항목 | 기본 구성 | TS 스택 (최적화 후) | 계층화 후 (청크) |
|---|---|---|---|
| 쓰기 (100행 배치) | 145,406 rows/s | **155,644 rows/s (+7%)** | — (적재 후 retier) |
| 읽기 latest (`LIMIT 1`) | 13,557–16,426 ops/s | 12,585 ops/s | **62 ops/s** † |
| 읽기 point (단건) | 12,021–14,777 ops/s | 13,925 ops/s | 4,190–4,438 ops/s (3.3×↓) |
| 읽기 range100 (100행 창) | 271,050–285,009 rows/s | 235,358 rows/s | **210,455–228,193 rows/s** (-11~-23%) |
| 디스크 (20M행, flush 후) | 237.9 MB | 237.9 MB | **123.4 MB (-48%)** = base 75.2 + 청크 48.3 |
| 행당 memtable 힙 | 기준 | **1/5.5** (92 B/row, 별도 실측) | — |

- 재인코딩(retier): 2,000만 행 318초 = **62.9k rows/s**, 청크 6,000개(500 태그 × 12 창).
- † **tiered `latest`는 실측된 심각한 병목**: 시간 경계 없는 `LIMIT 1`이 청크 창 목록을
  전 범위로 나열한 뒤에야 LIMIT을 세는 구조라 62 ops/s (p50 53ms, p99 180ms)까지
  떨어진다. 12워커×64 in-flight로 이 쿼리를 돌리면 노드가 포화돼 신규 연결까지
  타임아웃된다 — 벤치 중 세 차례의 "접속 불능"이 전부 이 증상이었다. 운영에서는
  hot_window가 최근 데이터를 원본으로 유지해 보통 회피되지만, **오래 전에 멈춘 태그의
  "현재값" 조회**가 정확히 이 최악 경로를 탄다. 수정 방향(창 목록 DESC+LIMIT 푸시다운)은
  SP4 Phase 1에 명세돼 있다.
- 구간 읽기(range100)는 청크에서도 -11~-23%만 잃는다 — 시계열 조회는 넓게 끊어 읽는
  패턴이 계층화와도 잘 맞는다.
- 관측 팁: 콜드 노드에 파이썬 드라이버(cqlsh 번들)로 동시 접속하면 control connection의
  기본 2초 타임아웃이 첫 시스템/스키마 읽기(디스크)에서 죽는다 — cqlsh(5초/10초)는 되고
  드라이버만 안 되는 모양새면 이것이다. `docker/rwbench-read.py`는 30초로 올려 두었다.
