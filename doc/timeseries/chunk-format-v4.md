...
**`statOrder`** — 통계를 계산한 비교 순서. `0x00 NONE` · `0x01 SIGNED_INT` · `0x02 UNSIGNED_INT`
...
3. **인코딩 선택**: 모든 선택은 **정확한 닫힌 형태** 크기 함수의 argmin, 동점은 최소 코드/최소 폭.
   시험 인코딩·타이밍·휴리스틱 금지.
4. **고정폭 메타데이터**: 헤더·디렉토리·블록 테이블의 모든 수치는 고정폭. varint는 데이터 본문에만
...
`1 | (2<<2) | (3<<4) = 0x31` → `00 00 00 00 00 00 00 31`.
