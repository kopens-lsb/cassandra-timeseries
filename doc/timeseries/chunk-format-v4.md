...
**예외 영역의 존재 신호는 인코딩마다 다르다**(구현 중 확정). `FOR_BITPACK`/`DELTA_FOR_BITPACK`은
blockFlags 비트 4를 쓴다. **`ALP`/`ALP_RD`는 비트 4를 항상 0으로 두고 본문 헤더의 `flags u8`
비트 0으로 알린다** — `DoubleBlockCodec` 심(seam)은 코덱이 예외 개수를 `Choice`로 되돌릴 통로가
없고 디코드 시 비트 4를 받지도 않기 때문이다. 길이 산술만으로 대신할 수는 없다: 본문 길이가 정확히
영역 크기만큼 줄어든 손상은 "예외 없는 더 짧은 본문"으로 **정합하게** 파싱되어 예외 행이 필러 0으로
조용히 디코드된다. 그래서 flags 비트와 파생 길이는 서로를 검증하며, 어긋나면 손상이다.
ALP 본문 레이아웃(헤더·프레임·레인·딕셔너리)은 `AlpBlockCodec` javadoc이 규범이다.

**폭 선택:** `cost(w) = ceil(n*w/64)*8 + (예외 있으면 8 + roundUp8(excCount*(2+W)))`를 65버킷
히스토그램 + 접미합으로 O(n+65)에 전부 계산 후 argmin, **동점은 최소 w**.
(이 문서의 초안은 고정항을 `16`이라고 적었는데, 바로 위 레이아웃의 헤더는
`excCount u16 | pad u16 | pad u32` = **8바이트**다. 둘 다 옳을 수는 없고 디스크의 바이트를
기술하는 쪽이 레이아웃이므로 `8`이 맞다. `16`으로는 예외를 가진 모든 폭이 8바이트씩 과대
계상되어 — 결정적이고 보수적이긴 하나 — 진짜 최소보다 한 칸 넓은 폭이 뽑힐 수 있었다.
`BitPacking.EXCEPTION_AREA_OVERHEAD`는 숫자를 다시 적지 않고 `ExceptionArea.HEADER_BYTES`를
참조하며, `ExceptionAreaTest`가 두 값의 일치를 단언한다 — 같은 모순이 두 번 생기지 않게.)
