...
image `cassandra-timeseries:6.0.0-tmtag` · table `scale.tm_tag_point` · single node in a container · 2026-08-01 11:32 UTC

- **30,000,000** rows loaded
- **600** tags (partitions), **50,000 rows per tag** = 13.9 h of history at 1 sample/1s
- **211 s** load time (142,180 rows/s)

## Query times

| section | query | first result row | CQL time |
|---|---|---|---|
| single partition, aggregates over value_numeric (tag-000001, type='double', 50,000 rows scanned) | count(*) [50,000 rows] | `50000` | **333 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 50,000 rows scanned) | time_bucket 1h + avg/min/max(value_numeric) [50,000 rows] | `2024-01-01 13:00:00 \| 13.7759 \| 13.3 \| 14.12` | **412 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 50,000 rows scanned) | time_bucket 5m + avg(value_numeric) [50,000 rows] | `2024-01-01 13:50:00 \| 13.377` | **577 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 50,000 rows scanned) | first/last/delta/rate(value_numeric) per hour [50,000 rows] | `2024-01-01 13:00:00 \| 13.73 \| 13.36 \| -0.37 \| -0.000115661` | **509 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 50,000 rows scanned) | derivative(value_numeric) per hour [50,000 rows] | `2024-01-01 13:00:00 \| -7.21185e-05` | **344 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 50,000 rows scanned) | percentile p50/p95/p99(value_numeric) [50,000 rows] | `13.54 \| 14.19 \| 14.34` | **419 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 50,000 rows scanned) | variance/stddev(value_numeric) [50,000 rows] | `0.166332 \| 0.407838` | **296 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 50,000 rows scanned) | histogram(value_numeric, 0, 100, 20) [50,000 rows] | `[0, 0, 0, 50000, 0, 0, 0,~` | **301 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 50,000 rows scanned) | approx_count_distinct(value_numeric) [50,000 rows] | `186` | **332 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 50,000 rows scanned) | integral + time_weighted_average(value_numeric) [50,000 rows] | `676893 \| 13.5381` | **4,970 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 50,000 rows scanned) | SECONDARY: time_bucket 1h + avg/min/max(latency) [50,000 rows] | `2024-01-01 13:00:00 \| 491 \| 1 \| 999` | **390 ms** |
| row reads (all 16 columns vs. projections) -- type-independent | newest 1000 rows, SELECT * -- type='boolean' tag (tag-000000) | `tag-000000 \| 2024-01-01 13:53:19 \| area-a \| asset-0000 \| line-01 \` | **60 ms** |
| row reads (all 16 columns vs. projections) -- type-independent | newest 1000 rows, SELECT * -- type='double' tag (tag-000001) | `tag-000001 \| 2024-01-01 13:53:19 \| area-b \| asset-0001 \| line-02 \` | **53 ms** |
| row reads (all 16 columns vs. projections) -- type-independent | 1 hour of rows, SELECT * (all columns) [3,600 rows] | `tag-000001 \| 2024-01-01 00:59:59 \| area-b \| asset-0001 \| line-02 \` | **213 ms** |
| row reads (all 16 columns vs. projections) -- type-independent | 1 hour of rows, project timestamp+value only [3,600 rows] | `2024-01-01 00:59:59 \| 13.58` | **55 ms** |
| row reads (all 16 columns vs. projections) -- type-independent | 1 hour of rows, project timestamp+value_numeric only [3,600 rows] | `2024-01-01 00:59:59 \| 13.58` | **54 ms** |
| row reads (all 16 columns vs. projections) -- type-independent | static columns only (1 row) | `tag-000001 \| busan \| area-b \| line-02 \| asset-0001 \| opc-01 \| PL` | **3 ms** |
| row reads (all 16 columns vs. projections) -- type-independent | column presence, type='boolean' tag [50,000 rows] | `50000 \| 50000 \| 0 \| 50000 \| 50000 \| 50000` | **358 ms** |
| row reads (all 16 columns vs. projections) -- type-independent | column presence, type='double' tag [50,000 rows] | `50000 \| 50000 \| 50000 \| 0 \| 50000 \| 50000` | **415 ms** |
| gap-fill (locf/interpolate over avg(value_numeric), numeric tag tag-000001) | gapfill 1h + locf over the full 13h span [50,000 rows] | `2024-01-01 00:00:00 \| 13.63` | **597 ms** |
| gap-fill (locf/interpolate over avg(value_numeric), numeric tag tag-000001) | gapfill 5m + interpolate over 6h [21,600 rows] | `2024-01-01 00:00:00 \| 13.7812` | **201 ms** |
| multi-partition (numeric-typed tags only -- 109 of 600 tags are numeric) | 10 numeric tags, hourly avg(value_numeric) [500,000 rows] | `tag-000001 \| 2024-01-01 13:00:00 \| 13.7759` | **4,429 ms** |
| multi-partition (numeric-typed tags only -- 109 of 600 tags are numeric) | 100 numeric tags, hourly avg(value_numeric) [5,000,000 rows] | `tag-000001 \| 2024-01-01 13:00:00 \| 13.7759` | **36,265 ms** |
| multi-partition (numeric-typed tags only -- 109 of 600 tags are numeric) | 100 numeric tags, p95(value_numeric) per tag [5,000,000 rows] | `tag-000001 \| 14.19` | **34,223 ms** |
| dashboard query (numeric tag tag-000001) | OHLC + change + p95 of value_numeric per hour [50,000 rows] | `2024-01-01 13:00:00 \| 3200 \| 13.73 \| 13.36 \| 13.3 \| 14.12 \| 13.7` | **526 ms** |
| full table scan (30,000,000 rows) -- WRONG ANSWER on a tiered table (known limitation) | count(*) over the whole table | `30000000` | **184,007 ms** |

## Details

### single partition, aggregates over value_numeric (tag-000001, type='double', 50,000 rows scanned)

**count(*) [50,000 rows]** — `333 ms`
...
**time_bucket 1h + avg/min/max(value_numeric) [50,000 rows]** — `412 ms`
...
**time_bucket 5m + avg(value_numeric) [50,000 rows]** — `577 ms`
...
**first/last/delta/rate(value_numeric) per hour [50,000 rows]** — `509 ms`
...
**derivative(value_numeric) per hour [50,000 rows]** — `344 ms`
...
**percentile p50/p95/p99(value_numeric) [50,000 rows]** — `419 ms`
...
**variance/stddev(value_numeric) [50,000 rows]** — `296 ms`
...
**histogram(value_numeric, 0, 100, 20) [50,000 rows]** — `301 ms`
...
**approx_count_distinct(value_numeric) [50,000 rows]** — `332 ms`
...
**integral + time_weighted_average(value_numeric) [50,000 rows]** — `4,970 ms`
...
**SECONDARY: time_bucket 1h + avg/min/max(latency) [50,000 rows]** — `390 ms`
...
**newest 1000 rows, SELECT * -- type='boolean' tag (tag-000000)** — `60 ms`
...
**newest 1000 rows, SELECT * -- type='double' tag (tag-000001)** — `53 ms`
...
**1 hour of rows, SELECT * (all columns) [3,600 rows]** — `213 ms`
...
**1 hour of rows, project timestamp+value only [3,600 rows]** — `55 ms`
...
**1 hour of rows, project timestamp+value_numeric only [3,600 rows]** — `54 ms`
...
**static columns only (1 row)** — `3 ms`
...
**column presence, type='boolean' tag [50,000 rows]** — `358 ms`
...
**column presence, type='double' tag [50,000 rows]** — `415 ms`
...
**gapfill 1h + locf over the full 13h span [50,000 rows]** — `597 ms`
...
**gapfill 5m + interpolate over 6h [21,600 rows]** — `201 ms`
...
**10 numeric tags, hourly avg(value_numeric) [500,000 rows]** — `4,429 ms`
...
**100 numeric tags, hourly avg(value_numeric) [5,000,000 rows]** — `36,265 ms`
...
**100 numeric tags, p95(value_numeric) per tag [5,000,000 rows]** — `34,223 ms`
...
**OHLC + change + p95 of value_numeric per hour [50,000 rows]** — `526 ms`
...
**count(*) over the whole table** — `184,007 ms`
