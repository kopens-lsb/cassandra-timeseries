...
### 5.1.2 ⚠️ 알려진 결함: 청크화된 구간의 `DELETE`는 **적용되지 않습니다**

**이미 청크화된 창을 대상으로 한 `DELETE`는 무시되며, 지운 데이터가 바로 다음 `SELECT`에서 다시
나옵니다.** 셀 단위·행 단위·클러스터링 레인지·파티션 전체 삭제 **모두** 해당합니다. 오류도, 경고도
없습니다.

원인은 병합 지점입니다. `SelectStatement`가 투명 읽기에 넘기는 것은 이미 **필터를 거친**
`PartitionIterator`입니다 — `db/transform/Filter`가 `purge(PURGE_ALL)`로 모든 톰스톤(셀·행·레인지·
파티션)을 제거하고 빈 행을 버린 뒤이므로, 청크 행을 병합하는 시점에는 삭제 정보가 이미 존재하지
않습니다. 볼 수 없는 삭제와는 조정(reconcile)할 수 없습니다.

필터 **이전**(unfiltered 스트림)에서 병합하는 방식도 구현해 측정했지만, 계층화의 핵심 불변식을 바꾸지
않고는 성립하지 않습니다: 재인코더는 원본 행을 `USING TIMESTAMP maxWt`로 지우는데, 청크가 복원하는
셀의 타임스탬프는 `max_row_writetime` = 같은 `maxWt`입니다. Cassandra의 삭제 판정은
`timestamp <= markedForDeleteAt`이므로 **재인코더 자신의 레인지 톰스톤이 청크가 복원한 모든 행을
가려버립니다**(실측: 병합 결과에 재인코더의 `INCL_START_BOUND@100` 마커와 사용자의 셀 톰스톤만 남고
복원된 값은 전부 사라짐).

**당분간의 운용 지침**: 계층화를 켠 테이블에서는 **핫 구간이 아닌 데이터를 `DELETE`하지 마십시오.**
콜드 데이터 삭제는 `cold_window` 만료를 쓰고, 파티션 전체를 지워야 한다면 베이스 테이블과
`<테이블>__chunks` 파티션을 **함께** 지우십시오. 해결 방향은 재인코더의 삭제 타임스탬프와 청크 복원
타임스탬프의 관계를 바꾸는 것이며, 별도 결정 사항으로 이월돼 있습니다.
