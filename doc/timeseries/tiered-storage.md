...
> **현 단계(SP2)의 범위**: 재인코딩·병합·만료·운영 도구까지입니다. 베이스 테이블 `SELECT`가 청크를
> 투명하게 합쳐 읽는 기능(투명 읽기)은 다음 단계(SP3)이며, 그 전까지 청크 데이터는
> [`__chunks` 테이블을 직접 조회](#3-청크-조회-패턴--sp3-투명-읽기-전까지)해야 합니다.
...
## 3. 청크 조회 패턴 — SP3 투명 읽기 전까지

재인코딩된 창의 행은 베이스 테이블에서 **삭제**되므로, 베이스 테이블 `SELECT`는 핫 구간만
