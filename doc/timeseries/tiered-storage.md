...
### 4.6 파킹된 컴팩션 창은 그 창의 계층화를 통째로 멈춥니다
...
실제로는 그 창이 FROZEN에 도달하지 못하고, 따라서 그 창에 대해 `WindowFrozenListener`가 **영원히
발화하지 않습니다** — 프리즈 뒤에 매달린 후속 처리(계층형 저장의 압축 포함)가 그 창에 한해 멈춥니다.
다른 창은 정상 진행하므로, 증상은 "테이블 전체가 멈춤"이 아니라 **특정 시간대만 영원히 압축되지
않음**입니다.
