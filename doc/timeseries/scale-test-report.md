...
| section | query | rows touched / result | CQL time |
|---|---|---|---|
| single partition (100000 rows) | count(*) | `100000` | **458 ms** |
| single partition (100000 rows) | time_bucket 1h + avg/min/max | `2024-01-01 00:00:00 \| 55.17 \| 10.0006 \| 95.9995` | **568 ms** |
| single partition (100000 rows) | time_bucket 5m + avg | `2024-01-01 00:00:00 \| 64.5966` | **382 ms** |
| single partition (100000 rows) | first/last/delta/rate per hour | `2024-01-01 00:00:00 \| 50 \| 82.698 \| 32.698 \| 0.0090853` | **438 ms** |
| single partition (100000 rows) | derivative per hour | `2024-01-01 00:00:00 \| -0.0128558` | **379 ms** |
| single partition (100000 rows) | percentile p50/p95/p99 (whole partition) | `53.3253 \| 92.7757 \| 95.0416` | **398 ms** |
| single partition (100000 rows) | variance/stddev (whole partition) | `805.694 \| 28.3847` | **267 ms** |
| single partition (100000 rows) | histogram 20 buckets | `[0, 0, 0, 8627, 10363, 6234, 5173, 4609, 4284, 4093, 3977, 3` | **283 ms** |
| single partition (100000 rows) | approx_count_distinct | `99741` | **292 ms** |
| single partition (100000 rows) | integral + time_weighted_average | `5.31023e+06 \| 53.1029` | **305 ms** |
| gap-fill | gapfill 1h + locf over the full span | `2024-01-01 00:00:00 \| 55.17` | **410 ms** |
| gap-fill | gapfill 5m + interpolate over 6h | `2024-01-01 00:00:00 \| 64.5966` | **96 ms** |
| multi-partition | 10 series, hourly avg (1000000 rows) | `sensor-000000 \| 2024-01-01 00:00:00 \| 55.17` | **3,633 ms** |
| multi-partition | 100 series, hourly avg (10000000 rows) | `sensor-000000 \| 2024-01-01 00:00:00 \| 55.17` | **36,067 ms** |
| multi-partition | 100 series, p95 per series | `sensor-000000 \| 92.7757` | **30,653 ms** |
| dashboard query | OHLC + change + p95 per hour | `2024-01-01 00:00:00 \| 3600 \| 50 \| 82.698 \| 10.0006 \| 95` | **497 ms** |
| full table scan (100000000 rows) | count(*) over the whole table | `100000000` | **238,510 ms** |

## Details

### single partition (100000 rows)
...
**time_bucket 1h + avg/min/max** — `568 ms`
...
**first/last/delta/rate per hour** — `438 ms`
...
**percentile p50/p95/p99 (whole partition)** — `398 ms`
...
**variance/stddev (whole partition)** — `267 ms`
...
**integral + time_weighted_average** — `305 ms`
...
```sql
SELECT time_bucket_gapfill(1h, ts, '2024-01-01 00:00:00+0000', '2024-01-02 03:46:40+0000'), locf(avg(value)) FROM scale.metrics WHERE series='sensor-000000' GROUP BY series, time_bucket_gapfill(1h, ts, '2024-01-01 00:00:00+0000', '2024-01-02 03:46:40+0000');
```

```
system_time_bucket_gapfill_1h__ts___2024_01_01_00_00_00_0000____2024_01_02_03_46_40_0000 | system_locf_system_avg_value
-----------------------------------------------------------------------------------------+-----------------------------
...
**gapfill 5m + interpolate over 6h** — `96 ms`
...
**10 series, hourly avg (1000000 rows)** — `3,633 ms`
...
**100 series, hourly avg (10000000 rows)** — `36,067 ms`
...
**100 series, p95 per series** — `30,653 ms`
...
**OHLC + change + p95 per hour** — `497 ms`
...
**count(*) over the whole table** — `238,510 ms`
