...
> 오버플로 재발 0건이고, 더 근본적으로 `pp.tm_tag_point`는 이제
> [`memtable = 'timeseries'`](timeseries-memtable.md)로 돌기 때문에 **64 MiB 상한 메커니즘
> 자체가 없습니다** — 창 배정이 flush가 아니라 쓰기 시점에 일어나 파티션을 버퍼링할 일이
> 없습니다. 아래 진단 절차는 **기본 memtable을 쓰는 테이블**에서 같은 증상을 만났을 때를
> 위해 남겨둡니다.
