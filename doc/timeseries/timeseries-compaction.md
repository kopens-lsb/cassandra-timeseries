...
### 실제로 파킹되는 이유는 대개 둘 중 하나입니다

2026-08-02 운영 노드에서 4개 테이블이 파킹됐고, 원인은 전부 아래 둘이었습니다. **원인을 먼저
가려야 합니다 — 대응이 정반대입니다.**

| 원인 | 파킹 메시지의 창 범위 | 대응 |
| --- | --- | --- |
| **쓰기 타임스탬프 오염** (`USING TIMESTAMP`에 µs 대신 ms) | **수십 년** | 오염 데이터 제거. `window_size`로는 불가능 |
| 64 MiB 라우팅 버퍼를 넘는 파티션 | 며칠~몇 주 | 파티션 축소(파티션 키에 시간 버킷) 또는 방치 |

진단은 파킹 메시지에 찍히는 창 범위로 합니다:

```
grep "Parking window" system.log | grep -oE '\[[0-9]+\.\.[0-9]+\]'
```

범위가 수십 년이면 오염입니다. `sstablemetadata`의 최소 타임스탬프가 16자리(µs)면 정상,
13자리면 ms를 µs로 해석한 것입니다.

두 번째 원인은 `WindowRoutingIterator.maxBufferedBytesPerPartition`(64 MiB)입니다. `SSTableWriter`가
파티션을 한 번에 받고 키를 한 번만 받으므로, 창 경계로 쪼개려면 파티션 전체를 힙에 올려야 합니다.
이 예산을 넘으면 라우팅이 분할을 포기하고 파티션을 통째로 씁니다 — 데이터는 온전하지만 결과가
창을 걸친 SSTable이라 모양이 안 변하고, 가드가 파킹합니다. **예산을 키우는 것은 권하지 않습니다**:
...
- **운영 실측 (2026-08-02, 단일 노드 448 GiB, 75개 테이블)**: 수명주기 전체를 분 단위로 압축해
