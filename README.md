...
| `time_bucket` + 집계 | 412~577 ms | **189~241 ms** | **2.2~2.4× 빠름** |
| gap-fill (locf / interpolate) | 201~597 ms | **46~143 ms** | **4.2~4.4× 빠름** |
| 100개 태그 시간별 평균 (500만 행) | 36.3 s | **17.3 s** | **2.1× 빠름** |
| 최신 1,000행 `SELECT *` | 53~60 ms | 167~212 ms | **3배 느림** |
| 재인코딩 처리량 | — | 64k rows/s | |

> **집계는 빨라지고 행 단위 조회는 느려집니다.** 청크는 창 단위로 디코드되므로, 5만 행을 훑는 대신
> 청크 하나를 읽는 집계는 이득이고 최신 N행만 꺼내는 패턴은 손해입니다. `static` 컬럼만 읽는 질의는
> 31배까지 느려집니다(벤치마크 문서 참고).
>
