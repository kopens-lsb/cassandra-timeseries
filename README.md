...
> 계층화의 값어치는 **디스크와 보존 기간**이지 지연 시간이 아닙니다. 위 수치는 압축에 **가장 불리한
> 모양**(고엔트로피 `double` 1개)에서 잰 것이라, 상수·전부-null 컬럼이 섞인 실 운영 테이블에서는
> 절감 폭이 더 큽니다 — [행당 바이트 실측](doc/timeseries/columnar-chunks.md) 참고.

> **`hot_window` 안쪽만 보는 질의는 비용이 없습니다.** 질의 범위가 콜드 경계 위에서 시작하면 병합
> 자체를 건너뛰고 핫 이터레이터를 그대로 돌려줍니다. 위의 손해는 **콜드 구간을 건드리는 질의만**
> 냅니다 — 대시보드가 최근 7일을 본다면 `hot_window: 7d`인 한 대시보드 속도는 계층화 전과 동일합니다.
